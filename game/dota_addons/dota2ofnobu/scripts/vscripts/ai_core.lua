...
function AICore:CreateBehaviorSystem( behaviors )
	...
	function BehaviorSystem:Think()
		...
		-- 接技
		if self.currentBehavior.orderTable then
			local ot = self.currentBehavior.orderTable
			local ot_count = #ot
			local time = 0
			for i,v in pairs(ot) do
				Timers:CreateTimer(time, function() 
					ExecuteOrderFromTable( v )
				end)
				time = time + 0.11
			end

		-- 不接技
		elseif self.currentBehavior.order and self.currentBehavior.order.OrderType ~= DOTA_UNIT_ORDER_NONE then
